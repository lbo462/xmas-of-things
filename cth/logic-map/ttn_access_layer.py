...
class TTNAccessLayer:
    ...
    Use instance.loop() to start listening for messages.
    """
    ...
    def _get_on_message(self, on_message_callback: Callable[[TTNMessage], None]):
        ...
        def on_message(client, userdata, msg):
            logger.debug(f"Message received on {self} (payload length: {len(msg.payload)})")

            payload = json.loads(msg.payload)
            decoded_payload = payload["uplink_message"]["decoded_payload"]
            ttn_message = TTNMessage.from_dict(decoded_payload)
            logger.debug(f"Payload decoded, triggering callback function with {ttn_message}")
